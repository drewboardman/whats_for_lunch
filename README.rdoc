...
* We need a way to add new restaurants to our app's database
    The add feature is available on the bottom of the dashboard show view(root) and also in a link at the top navbar. Restaurants added do not belong to a single user and can be seen by all users.
* We need to be able to give each restaurant an overall rating on a scale of 1-5, with 1 being the lowest and 5 the highest.
    The rating feature is available in all views of the restaurant. There is a Rating model that belongs_to both User and Restaurant.
